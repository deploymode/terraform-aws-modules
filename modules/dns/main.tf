...
module "acm_request_certificate" {
  source                            = "git::https://github.com/cloudposse/terraform-aws-acm-request-certificate.git?ref=tags/0.9.0"
  ...
  # local.zone_name_in_use # var.create_hosted_zone ? join("", aws_route53_zone.default.*.name) : join("", data.aws_route53_zone.existing.*.name)
